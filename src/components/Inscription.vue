...
const submitForm = async () => {
	const detection = await getFacialInformation();
	if (!detection) {
	...
	try {
		const imgBuffer = canvasRef.value.toDataURL('image/jpeg');
		const bufferToFile = (dataURI, filename) => {
		...
		const file = bufferToFile(imgBuffer, 'photo.jpg');

		const formData = new FormData();
		formData.append('email', email.value);
		formData.append('descriptor', descriptorToString);
		...
		console.log({
			...
			photoBuffer: canvasRef.value.toDataURL('image/jpeg'),
			response,
		});
