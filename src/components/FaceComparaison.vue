  ...
  export default {
    setup() {
      ...
      const capture = () => {
        ...
        context.drawImage(videoRef.value, 0, 0, videoRef.value.videoWidth, videoRef.value
          .videoHeight)
        const imageData = context.getImageData(0, 0, canvasRef.value.width, canvasRef.value
          .height)
        if (!image1.value) {
      ...
      const compare = async () => {
        ...
        const detection1 = await faceapi.detectSingleFace(canvas).withFaceLandmarks()
          .withFaceDescriptor()
        canvas.width = image2.value.width;
        canvas.height = image2.value.height;
        context.putImageData(image2.value, 0, 0);
        const detection2 = await faceapi.detectSingleFace(canvas).withFaceLandmarks()
          .withFaceDescriptor()
